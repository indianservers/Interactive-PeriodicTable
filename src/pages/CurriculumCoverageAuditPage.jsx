...
const CoveragePreview = ({ strands }) => (
  <svg
    viewBox="0 0 520 190"
    className="h-56 w-full rounded-2xl border border-white/10 bg-black/20"
  >
    ...
    <text x="38" y="24" fill="#cbd5e1" fontSize="12">
      Filtered curriculum coverage by strand
    </text>
  </svg>
);
  ...
  const filteredStrands = useMemo(
    () =>
      curriculumStrands.filter((strand) => {
        ...
        const gradeMatch =
          activeGradeBand === "all" ||
          strand.gradeBand === activeGradeBand ||
          strand.gradeBand === "all";
        const queryMatch =
          !query ||
          `${strand.title} ${strand.nextBuild}`
            .toLowerCase()
            .includes(query.toLowerCase());
        return boardMatch && gradeMatch && queryMatch;
      }),
    [activeBoard, activeGradeBand, query],
  );

  const activeStrand =
    filteredStrands.find((strand) => strand.id === activeStrandId) ||
    filteredStrands[0] ||
    curriculumStrands[0];
  const averageCoverage = Math.round(
    filteredStrands.reduce((total, strand) => total + strand.coverage, 0) /
      Math.max(filteredStrands.length, 1),
  );
  const activeBoardLabel =
    boardFilters.find((board) => board.id === activeBoard)?.label ||
    "All Boards";

  const selectBoard = (boardId) => {
    setActiveBoard(boardId);
    const first = curriculumStrands.find(
      (strand) => boardId === "all" || strand.boards.includes(boardId),
    );
    setActiveStrandId(first?.id || activeStrandId);
    announce(
      `${boardFilters.find((board) => board.id === boardId)?.label || "All boards"} filter selected`,
    );
  };
  ...
  const selectGradeBand = (gradeBandId) => {
    ...
    const first = curriculumStrands.find(
      (strand) =>
        (activeBoard === "all" || strand.boards.includes(activeBoard)) &&
        (gradeBandId === "all" ||
          strand.gradeBand === gradeBandId ||
          strand.gradeBand === "all"),
    );
    setActiveStrandId(first?.id || activeStrandId);
    announce(`${gradeLabel[gradeBandId]} filter selected`);
  };
    ...
    <div className="mx-auto h-[calc(100vh-70px)] max-w-7xl space-y-4 overflow-hidden p-4 md:p-6">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-gray-400">
              AP State, CBSE, IGCSE and IB Chemistry from grades 6-12, mapped to
              interactive 2D, 3D, lab, practice and teacher-ready learning
              paths.
            </p>
          </div>
          <div className="grid grid-cols-5 gap-2 text-center">
            {[
              ...
              <div
                key={label}
                className="rounded-xl border border-white/10 bg-white/[0.055] px-3 py-2"
              >
      ...
      <section className="grid gap-4 xl:grid-cols-[310px_1fr]">
        <aside className="glass h-fit rounded-2xl p-4">
          ...
          <label className="mt-4 flex items-center gap-2 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-gray-400">
            <Search size={14} className="text-cyan-300" />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search strands or gaps"
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-3 text-center">
            ...
            <p className="mt-1 text-xs text-gray-500">
              {gradeLabel[activeGradeBand]} average coverage
            </p>
          </div>
        </aside>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-cyan-300">
                    {filteredStrands.length} matching strands
                  </p>
                ...
                <button
                  type="button"
                  onClick={() => onNavigate?.("learning-command")}
                  ...
                  >
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10">
                      <div
                        ...
                        style={{ width: `${board.coverage}%` }}
                      />
                    </div>
                    <p className="mt-2 text-xs text-gray-500">
                      Needs: {board.needs.join(", ")}
                    </p>
                  </div>
                ))}
              </div>
                ...
                {filteredStrands.map((strand) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      announce(`${strand.title} strand selected`);
                    }}
                    className={`w-full rounded-xl border p-3 text-left transition-colors ${
                      activeStrand.id === strand.id
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      {gradeLabel[strand.gradeBand] || "Cross-grade"} |{" "}
                      {strand.boards.length} board maps
                    </p>
                  </button>
                ))}
              </div>
            ...
            <div className="glass rounded-2xl p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                ...
                <button
                  type="button"
                  onClick={() => {
                    announce(`Opening ${activeStrand.title}`);
                    onNavigate?.(activeStrand.route);
                  }}
                  className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-xs font-black text-emerald-100 hover:bg-emerald-300/15"
                >
                ...
                <div className="rounded-xl border border-white/10 bg-black/15 p-3">
                  ...
                  <div className="space-y-1.5">
                    {activeStrand.missing.map((item) => (
                      <p
                        key={item}
                        className="rounded-lg border border-amber-300/10 bg-amber-300/5 px-2 py-1 text-[11px] font-bold text-amber-100"
                      >
                        {item}
                      </p>
                    ))}
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                <div
                  className={`rounded-xl border px-3 py-2 text-xs font-black ${readinessClass[activeStrand.twoD] || readinessClass.Planned}`}
                >
                  <Box size={14} className="mb-1" /> 2D: {activeStrand.twoD}
                </div>
                <div
                  className={`rounded-xl border px-3 py-2 text-xs font-black ${readinessClass[activeStrand.threeD] || readinessClass.Planned}`}
                >
                  <Layers3 size={14} className="mb-1" /> 3D:{" "}
                  {activeStrand.threeD}
                </div>
              ...
              <p className="mt-3 rounded-xl border border-white/10 bg-white/[0.035] p-3 text-sm text-gray-300">
                {activeStrand.nextBuild}
              </p>
            </div>
          </section>
            ...
            <div className="grid gap-3 lg:grid-cols-5">
              {priorityGapBacklog.map((gap) => (
                <button
                  key={gap.id}
                  type="button"
                  onClick={() => onNavigate?.(gap.route)}
                  className="rounded-xl border border-white/10 bg-white/[0.035] p-3 text-left hover:bg-white/[0.065]"
                >
                  <p className="text-sm font-black text-white">{gap.title}</p>
                  <p className="mt-1 text-[11px] font-bold text-cyan-200">
                    {gap.board} | {gap.impact}
                  </p>
                  <p className="mt-2 text-xs text-gray-500">
                    {gap.tasks.slice(0, 2).join(", ")}
                  </p>
                </button>
              ))}
            </div>
                ...
                {contentEnhancementPlan.map((item) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className="rounded-full border border-amber-300/20 bg-amber-300/10 px-2 py-0.5 text-[10px] font-black text-amber-100">
                        {item.priority}
                      </span>
                    </div>
          ...
          <section className="grid gap-4 xl:grid-cols-[1fr_0.85fr]">
            ...
            <div className="glass rounded-2xl p-4">
              <div className="mb-3 flex items-center gap-2">
                <BookOpen size={16} className="text-cyan-300" />
                <h3 className="text-sm font-bold text-white">
                  Telugu / Hindi / English Starter Glossary
                </h3>
              </div>
              <div className="space-y-2">
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <span className="rounded-lg border border-cyan-300/15 bg-cyan-300/10 px-2 py-0.5 text-[11px] font-black text-cyan-100">
                          {term.te}
                        </span>
                        <span className="rounded-lg border border-emerald-300/15 bg-emerald-300/10 px-2 py-0.5 text-[11px] font-black text-emerald-100">
                          {term.hi}
                        </span>
                      </div>
                    </div>
